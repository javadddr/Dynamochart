...
const xScale = d3.scalePoint()
    ...
    const yScale = d3.scaleLinear()
      .domain([0, d3.max(transformedData1.flat(), d => d.value)])
...
// Append x-axis title
    svg.append("text")
      ...
      .attr("fill", chartTemplate==="t1"?"black":chartTemplate==="t2"?"rgb(216, 221, 43)":"white")
      .text(xAxisTitle);
...
yAxisGroup.append("text")
  ...
  .text(yAxisTitle);
   ...
   // Add legend title
    ...
    .enter().append('g')
    .attr('class', 'legend')
    .attr('transform', (d, i) => `translate(${width - margin.right + legendMargin.right}, ${legendMargin.top + i * legendHeight + legendSquareSize})`) // Position based on the new margin
...
d3.select('#tooltipidk').style('display', 'none');
  ...
  return (
    <div style={{ width:dimensions.width>chartWidth?chartWidth:dimensions.width, height :dimensions.height>chartHeight?chartHeight:dimensions.height,border:chartBorder?"1px solid black":"none" ,padding:"10px",margin:"10px",backgroundColor:chartTemplate==="t1"?"white":chartTemplate==="t2"?"#1F1F1F":"white"}}>
      <svg ref={svgRef} className="mySvgStyle" style={{backgroundColor:chartTemplate==="t1"?"white":chartTemplate==="t2"?"#1F1F1F":"white"}}></svg>
      <div id="tooltipidk" className="tooltip-linopd" style={{ display: 'none' }}></div>
    </div>
  );
